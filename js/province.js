...
async function selectPhuongFromPoint(lng, lat, map) {
    // Lấy phần tử thẻ chọn Tỉnh và Phường/Xã trên giao diện HTML
    ...
    let matchedProvince = null; // Biến lưu thông tin tỉnh khớp với điểm click
    ...
    if (matchedProvince && targetGeoData) {
        ...
        if (tinhSelect.value !== matchedProvince.id) {
            tinhSelect.value = matchedProvince.id; // Gán giá trị mã tỉnh vào thẻ select giao diện
            ...
            // Sắp xếp thứ tự tên phường/xã theo bảng chữ cái A-Z và đưa vào các thẻ option
            ...
            // Khi click trực tiếp trên bản đồ trúng vị trí, ta đồng thời tải luôn dữ liệu thửa đất từ Google Sheets tương ứng
        ...
        if (matchedPhuong && phuongSelect) {
            phuongSelect.value = matchedPhuong; // Gán giá trị phường/xã vào thẻ select
            ...
            // Áp dụng bộ lọc hiển thị lên các lớp bản đồ ranh giới và thửa đất
...
async function loadProvinceData(provinceId, map) {
    // Lấy phần tử dropdown chọn Phường/Xã trên giao diện
    const phuongSelect = document.getElementById('phuongFilter');
    ...
    try {
        ...
        // Tự động zoom màn hình ôm khít tỉnh với lề đệm 50px và tốc độ chuyển động mượt 300ms
    ...
    // 🛑 ĐÃ LƯU Ý: Đã lược bỏ hoàn toàn lệnh `await loadThuaDatFromSheet(map);` tại đây. 
    // Việc này giúp khi chọn tỉnh từ dropdown, bản đồ KHÔNG gọi tải dữ liệu thửa đất nữa để tăng tốc độ tối đa!
}
...
function initFilter(map) {
    ...
    phuongSelect.addEventListener('change', async (e) => {
        ...
        if (!selectedPhuong) {
            hideThuaDat(map); // Ẩn các lớp dữ liệu thửa đất đi khi bỏ chọn phường
            ...
            // Áp dụng bộ lọc vào các lớp bản đồ ranh giới tỉnh
            if (map.getLayer('thua-dat-layer')) map.setFilter('thua-dat-layer', filterExpr);
            if (map.getLayer('thua-dat-line-layer')) map.setFilter('thua-dat-line-layer', filterExpr);
            
            // ✅ CHỈ KHI CHỌN PHƯỜNG/XÃ TỪ DROPDOWN THÌ MỚI GỌI TẢI DỮ LIỆU THỬA ĐẤT TỪ GOOGLE SHEETS
            await loadThuaDatFromSheet(map);

            // Áp dụng bộ lọc cho các lớp thửa đất từ Google Sheets
            if (map.getLayer('sheet-thua-dat-fill')) map.setFilter('sheet-thua-dat-fill', sheetFilterExpr);
            if (map.getLayer('sheet-thua-dat-line')) map.setFilter('sheet-thua-dat-line', sheetFilterExpr);
            ...
            if (currentGeoData) {
                ...
                if (filtered.length > 0) {
                    ...
                    map.fitBounds(bbox, { padding: 50, duration: 300 }); // Phóng to màn hình vừa vặn với ranh giới xã, tốc độ 300ms
                }
            }
        }
    });
