...
/**
 * 4. TÍNH NĂNG TÌM KIẾM THEO TỈNH ĐANG CHỌN (HỖ TRỢ TÌM SỐ TỜ/SỐ THỬA, TÊN CHỦ, MÃ ĐỊNH DANH TRÊN TOÀN TỈNH)
 */
function executeProvinceSearch() {
    const tinhSelect = document.getElementById('tinhFilter');
    if (!tinhSelect || !tinhSelect.value) {
        alert("Vui lòng chọn Tỉnh / Thành phố trước khi thực hiện tìm kiếm!");
        return;
    }
    ...
    if (!keyword) {
        // Nếu ô tìm kiếm trống, reset bộ lọc thửa đất về trạng thái rỗng
    ...
    // Biểu thức lọc MapLibre trên lớp dữ liệu Google Sheets (hỗ trợ tìm số tờ, số thửa, tên chủ, mã định danh, ID)
    const searchFilter = [
        'any',
    ...
    // Quét dữ liệu trong source của bản đồ để tính toán khung bao (bounding box) và zoom tới tập hợp kết quả tìm thấy trong tỉnh
    const source = mapInstance.getSource('sheet-thua-dat-src');
    // Lưu ý: source chứa ranh giới GeoJSON của tỉnh, còn dữ liệu thửa đất từ Google Sheets thường nằm ở source riêng (ví dụ 'sheet-thua-dat-src'). Ta check linh hoạt cả 2 nguồn:
    let sheetSource = mapInstance.getSource('sheet-thua-dat-src') || mapInstance.getSource('sheet-thua-dat');
    
    // Tìm kiếm trong tất cả các nguồn dữ liệu có sẵn của bản đồ
    let matchedFeatures = [];
    
    // Duyệt qua source thửa đất của sheet nếu có
    const targetSource = mapInstance.getSource('sheet-thua-dat-src'); // hoặc tên source sheet của hệ thống ông
    // Thường hệ thống dùng chung hoặc source riêng, ta kiểm tra trực tiếp qua các features đang có trên layer sheet
    
    // Cách an toàn nhất là quét qua danh sách features từ source sheet
    if (targetSource && targetSource._data && targetSource._data.features) {
        matchedFeatures = targetSource._data.features.filter(f => {
            const p = f.properties || {};
    ...
    if (matchedFeatures.length > 0) {
        ...
        // Nếu không tìm thấy trong source trực tiếp, bản đồ vẫn áp dụng filter hiển thị (nếu dữ liệu chuẩn khớp filter maplibre)
        // Thông báo nhẹ để người dùng nắm bắt
        console.log("Đã áp dụng bộ lọc tìm kiếm cho từ khóa: " + keyword);
    }
}

// Gắn sự kiện lắng nghe cho Nút Tìm Kiếm và phím Enter khi DOM đã sẵn sàng
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => {
        const searchBtn = document.getElementById('searchBtn'); // ID của nút tìm kiếm trên giao diện HTML của ông
        const searchInput = document.getElementById('searchInput');
